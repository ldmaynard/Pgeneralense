...
#SRW: For some reason when I load the file the header for "Casa" is messed up
colnames(grow)[1] <- "Casa"

grow$rel_gro<-((grow$ht.2018.09.cm-grow$ht.2018.04.cm)/grow$ht.2018.04.cm)

#SRW: also creating variable for total growth
...
#SRW: checking distributions for relative and total growth
hist(grow$rel_gro)  #not normal, this is a typical proportion
hist(grow$total_gro)

#SRW: re-ordering factor levels so lm will compare everything to control
...
#SRW: yes, here each Casa is a replicate, so you would not need to block for that

gro2<-aov(grow$rel_gro~grow$Treatment)
summary.aov(gro2)
#p=0.927

#SRW: trying a few more models with lms and betareg
...
#SRW: checking distribution
hist(phen_ag2$pdw) #wow this looks great

#models for pdw
phen1<-lmer(pdw ~ treat * stage + (1|chamber), data=phen_ag2, na.action = "na.omit", REML=F)
phen2<-lmer(pdw ~ treat + stage + (1|chamber), data=phen_ag2, na.action = "na.omit", REML=F)
phen3<-lmer(pdw ~ treat + (1|chamber), data=phen_ag2, na.action = "na.omit", REML=F)
phen4<-lmer(pdw ~ stage + (1|chamber), data=phen_ag2, na.action = "na.omit", REML=F)
phen.null<-lmer(pdw ~ 1 + (1|chamber), data=phen_ag2, na.action = "na.omit", REML=F)
...
#SRW: I added it to the model statements above and it worked, though now
#you have a singular fit error with the treatment only model
...
lmm1<-lmer(prop_herb ~ age + (1|chamber), data=pg1, na.action = "na.omit", REML=F)
lmm2<-lmer(prop_herb ~ treatment + (1|chamber), data=pg1, na.action = "na.omit", REML=F)
lmm3<-lmer(prop_herb ~ age + treatment + (1|chamber), data=pg1, na.action = "na.omit", REML=F)
lmm4<-lmer(prop_herb ~ age * treatment + (1|chamber), data=pg1, na.action = "na.omit", REML=F)#warning
lmm5<-lmer(prop_herb ~ (1|chamber), data=pg1, na.action = "na.omit", REML=F)
...
#SRW: I added REML=F to all model statements above, that changed the results
#some. Age is still best but add is a close second with dAIC=0.75
...
lmm1t<-lmer(pLogit ~ age + (1|chamber), data=pg1, na.action = "na.omit", REML=F)
lmm2t<-lmer(pLogit ~ treatment + (1|chamber), data=pg1, na.action = "na.omit", REML=F)
lmm3t<-lmer(pLogit ~ age + treatment + (1|chamber), data=pg1, na.action = "na.omit", REML=F)
lmm4t<-lmer(pLogit ~ age * treatment + (1|chamber), data=pg1, na.action = "na.omit", REML=F)#warning
lmm5t<-lmer(pLogit ~ (1|chamber), data=pg1, na.action = "na.omit", REML=F)

modcomp.lmmt<-aictab(cand.set=list(lmm1t, lmm2t, lmm3t, lmm4t, lmm5t),
					modnames=c("age", "treat", "add", "interactive", "null"), REML=F)#AIC table
modcomp.lmmt#error about fixed effects bening different?
#best model is age, next model = null and dAIC>3
#SRW: added REML=F to all models, that got rid of errors with model comparison
#but now there are some singular fit errors
...
#SRW: with changes above: AGE is now best fit, and additive is close behind

#SRW: checking additive model
summary(herb.3)
drop1(herb.3, test="Chisq") #strong effect of age and marginal effect of treatment
...
lmm20<-lmer(prop_herb ~ age + (1|chamber), data=pg.herb.pres, na.action = "na.omit", REML=F)
lmm21<-lmer(prop_herb ~ treatment + (1|chamber), data=pg.herb.pres, na.action = "na.omit", REML=F)
lmm22<-lmer(prop_herb ~ age + treatment + (1|chamber), data=pg.herb.pres, na.action = "na.omit", REML=F)
lmm23<-lmer(prop_herb ~ age * treatment + (1|chamber), data=pg.herb.pres, na.action = "na.omit", REML=F)
lmm24<-lmer(prop_herb ~ (1|chamber), data=pg.herb.pres, na.action = "na.omit", REML=F)

modcomp.lmm2<-aictab(cand.set=list(lmm20, lmm21, lmm22, lmm23, lmm24),
					modnames=c("age", "treat", "add", "interactive", "null"), REML=F)#AIC table
modcomp.lmm2
#best model is null, next model = age and dAIC>8
#SRW: added REML=F to all models

#SRW: I added chamber to this aggregate statement
pg.herb.pres2<-aggregate(prop_herb~chamber + treatment + age,data=pg.herb.pres,FUN=mean)
...
#SRW: model with treatment is now best

#SRW: checking model with treatment
summary(lm26)
...
#SRW: I did not really look carefully from here down
